...
- **SEO report viewer** (`components/seo/`): 11 komponent pro zobrazení výsledků technické SEO analýzy.
  ...
  - `filtered-category-section.tsx` — Client component: filtrovaný Accordion per kategorie. useMemo s 3-level filtrací (severity, category, text). Počítadlo "N z M nálezů". Empty state "Žádné nálezy neodpovídají filtru." + "Zrušit filtry" button.
  - `issue-card.tsx` — Client component: severity border-left, badge, description, recommendation box (Lightbulb), Collapsible affected URLs (max 20). Volitelný `onClick` prop s ChevronRight ikonou, hover shadow, keyboard a11y (Enter/Space).
  - `issue-detail-sheet.tsx` — Client component: Sheet side="right" (w-full sm:max-w-lg). Popis, doporučení, Impact/Effort badges (podmíněno scoredIssue), quadrant badge (Quick Win/Major Project/Fill-in/Time Waster), scrollable affected URLs (max 50, external links).
